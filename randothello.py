...
class MinimaxPlayer(OthelloPlayerTemplate):
    ...
    def min_value(self, s, depth):
        best_move = None
        if terminal_test(s) or depth == 0:
            return self.utility(s), None
        min_value = float('inf')
        for action in actions(s):
            result_state = result(s, action)
            v, a = self.max_value(result_state, depth - 1)
            if v < min_value:
                min_value = v
                best_move = action
        return min_value, best_move

    def max_value(self, s, depth):
        best_move = None
        if terminal_test(s) or depth == 0:
            return self.utility(s), None
        max_value = float('-inf')
        for action in actions(s):
            result_state = result(s, action)
            v, a = self.min_value(result_state, depth - 1)
            if v > max_value:
                max_value = v
                best_move = action
    ...
    def minimax(self, state, depth):
        ...
        val, b_move = self.max_value(state, depth)
        display(state)
        return b_move
...
class AlphaBetaPlayer(OthelloPlayerTemplate):
    ...
    def max_value_ab(self, s, alpha, beta, depth):
        best_move = None
        if terminal_test(s) or depth == 0:
            return self.utility(s), None
        val = float('-inf')
        for action in actions(s):
            result_state = result(s, action)
            v, _ = self.min_value_ab(result_state, alpha, beta, depth - 1)
            if v > val:
                val, move = v, action
            alpha = max(val, alpha)
            if val >= beta:
    ...
    def min_value_ab(self, s, alpha, beta, depth):
        best_move = None
        if terminal_test(s) or depth == 0:
            return self.utility(s), None
        val = float('inf')
        for action in actions(s):
            result_state = result(s, action)
            v, _ = self.max_value_ab(result_state, alpha, beta, depth - 1)
            if v < val:
                val, move = v, action
            beta = min(val, beta)
            if val <= alpha:
    ...
    def alphabeta(self, state, depth):
        ...
        val, b_move = self.max_value_ab(state, float('-inf'), float('inf'),depth)
        display(state)
        return b_move
...
def play_game(p1 = None, p2 = None):
    ...
    '''
    if p1 == None:
        p1 = AlphaBetaPlayer(BLACK)
    if p2 == None:
        p2 = RandomPlayer(WHITE)

    s = RandOthelloState(p1, p2)
    while True:
        action = p1.alphabeta(s, 4)
        ...
        if terminal_test(s):
            ...
            display_final(s)
            return
def play_game1(p1 = None, p2 = None):
    '''Plays a game with two players. By default, uses two humans
    '''
    if p1 == None:
        p1 = AlphaBetaPlayer(WHITE)
    if p2 == None:
